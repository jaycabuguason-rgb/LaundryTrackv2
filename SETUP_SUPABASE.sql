...
-- Staff management sync and realtime-friendly profile fields for LaundryTrack
...
select
  ...
  case
    ...
    ) = 'admin' then 'admin'::public.user_role
    else 'staff'::public.user_role
  end,
  coalesce(
    (users.raw_app_meta_data ->> 'is_active')::boolean,
    (users.raw_user_meta_data ->> 'is_active')::boolean,
    true
  )
from auth.users as users
