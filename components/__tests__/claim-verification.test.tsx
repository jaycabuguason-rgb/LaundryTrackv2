...
describe("ClaimVerificationPage Name Suggestions", () => {
  it("suggests matching customer names when typing the first letter", () => {
    render(
    ...
    // Suggestions dropdown should appear with Juan Dela Cruz and Jane Doe, but not Alice Smith
    ...
    expect(options[1]).toHaveTextContent("#TKT-0031");
  });
  ...
  it("navigates suggestions with arrow keys and selects with Enter", () => {
    ...
    // Press Down Arrow to highlight second item (Jane)
    fireEvent.keyDown(input, { key: "ArrowDown" });

    // Press Enter to select
    fireEvent.keyDown(input, { key: "Enter" });

    // Suggestions dropdown closes and Jane Doe's ticket is loaded
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
    expect(screen.getAllByText(/TKT-0031/).length).toBeGreaterThan(0);
